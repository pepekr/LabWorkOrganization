...
{
    public class CourseScopedClient<TEntity, TResponse> : ClassroomClient<TEntity, TResponse>,
        ICourseScopedExternalRepository<TEntity>
        where TEntity : class, IHasCourseId
    {
        public CourseScopedClient(HttpClient client, string baseUrl, IMapper mapper,
            IExternalTokenProvider extTokenProvider)
        ...
        public async Task<IEnumerable<TEntity>> GetAllByCourseIdAsync(string courseId)
        {
            ...
            HttpResponseMessage res = await _httpClient.GetAsync($"{_baseUrl}?courseId={courseId}");
