...
namespace LabWorkOrganization.Application.Services
{
    public class CourseService : ICourseService
    {
        ...
        private ICourseService _courseServiceImplementation;
        ...
        #region v2
        public async Task<Result<Course>> CreateCourseV2(CourseCreationalDtoV2 course, bool useExternal)
        {
            try
            {
                List<string> errors = ValidationHelper.Validate(course);
                ...
                Course newCourse = new()
                {
                    ...
                    OwnerExternalId = user.Data.SubGoogleId,
                    Name = course.Name,
                    // якщо впадлу вводити опис, в бд буде null, а на фронт передаватиметься пустий рядок через dto.
                    Description = string.IsNullOrWhiteSpace(course.Description) ? null : course.Description,
                    LessonDuration = course.LessonDuration,
                    EndOfCourse = course.EndOfCourse
                };
                if (useExternal)
        ...
        public async Task<Result<Course?>> GetCourseByIdV2(string id, bool external = false)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(id))
        ...
        public async Task<Result<IEnumerable<Course>>> GetAllCoursesV2(bool isGetExternal = false)
        {
            try
            {
                IEnumerable<Course>? courses = await _crudRepository.GetAllAsync();
        ...
        //**********************FIX THIS <3 ***********************************************************
        public async Task<Result<IEnumerable<Course>>> GetAllCoursesByUserAsyncV2(bool includeExternal = false)
        {
            try
            {
        ...
        public async Task<Result<Course>> UpdateCourseV2(CourseUpdateDtoV2 course, bool updateExternal = false)
        {
            try
            {
                List<string> errors = ValidationHelper.Validate(course);
        ...
        public async Task<Result<Course>> DeleteCourseV2(string id)
        {
            try
            {
                string userId = _userService.GetCurrentUserId();
